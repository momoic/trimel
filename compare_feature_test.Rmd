...
logrank <- survdiff(Surv(Met_Time, Mets) ~ Sex, data = data)
```
...
```{r}
data_new = read.csv("C://PhD work//trimel//artificial_roswell-EOC+BCHI.csv")
...
high_risk = subset(data_new, data_new$Group %in% subset(data2_new, Risk == "HR")$Cluster)
...
low_risk = subset(data_new, data_new$Group %in% subset(data2_new, Risk == "LR" & Significance == "S")$Cluster)
