...
opt_gmm = Optimal_Clusters_GMM(data.ml, max_clusters = 50, criterion = "BIC",
                              ...
                              km_iter = 5, em_iter = 100, var_floor = 1e-10,
...
gmm = GMM(data.ml, index, dist_mode = "eucl_dist", seed_mode = "random_subset", km_iter = 10,
...
low_risk_clusters = (low_risk %>%
      ...
      mutate(N = sum(n), p = 100*n/N) %>%
      group_by(Group) %>%
      summarize(ratio = p[Mets == "Alive" & Breslow_Depth == "High"]/p[Mets == "Alive" & Breslow_Depth == "Low"]) %>%
...
gmm = GMM(data_new.ml, index, dist_mode = "eucl_dist", seed_mode = "random_subset", km_iter = 5,
