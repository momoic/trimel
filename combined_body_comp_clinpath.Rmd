...
data <- read.csv("C://PhD work//Melanoma-ClinPath-Model//Datasets//SEER_curated_5years.csv")[1:2000,-1]
...
opt_gmm = Optimal_Clusters_GMM(data, max_clusters = 50, criterion = "BIC",

                              dist_mode = "eucl_dist", seed_mode = "random_subset",

                              km_iter = 10, em_iter = 5, var_floor = 1e-10,

                              plot_data = T)
plot(1:50, opt_gmm)


gmm = GMM(data, 40, dist_mode = "maha_dist", seed_mode = "random_subset", km_iter = 10,
...
```{r}
inTraining = createDataPartition(data$Age, p = 0.8, list = FALSE)
train_data = data[inTraining,]
testing = data[-inTraining,]

fitControl = trainControl(method = "boot", number = 10, repeats = 10)
```


```{r}
modelLookup(model = "glmboost")

nn_model = train(Group~., data = train_data, method = "nnet",trControl = fitControl, tuneGrid = expand.grid(.size = c(1,5,10), .decay = 0,0.001,0.1))
...
svm_model = train(Group~., data = train_data, method = "svmLinearWeights", trControl = fitControl)
xgb_model = train(Group~., data = train_data, method = "xgbLinear", trControl = fitControl, tuneGrid = expand.grid(.nrounds = 300, .eta = c(0.01, 0.1, 1)))
...
glm_boost_model = train(Group~., data = train_data, method = "glmboost", trControl = fitControl, expand.grid(.mstop = 300, prune = ))
