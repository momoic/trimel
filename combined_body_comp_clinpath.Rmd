...
comb_data$combined_risk[comb_data$Risk == "Low Risk" & comb_data$BCHI < res.cut[[1]][[4]][[1]]] = "Low-Low"
...
comb_data$combined_risk[comb_data$Risk == "Low Risk" & comb_data$BCHI >= res.cut[[1]][[4]][[1]]] = "Low-High"
...
ggsurvplot(fit, risk.table = TRUE, conf.int = FALSE, pval = TRUE)
pairwise_survdiff(Surv(MSS_Time, MSS) ~ combined_risk, data = comb_data, p.adjust.method = "bonferroni")
```
...
same = 0
not = 0
for (i in 1:nrow(data_retrain)){
  if(data_retrain$Group[i] == data$Group[i]){
    same = same + 1
  }
  else{
    not = not+1
...
```
```{r}
pairwise_survdiff(Surv(MSS_Time, MSS) ~ Risk, data = risk_data_retrain, p.adjust.method = "bonferroni")
```
